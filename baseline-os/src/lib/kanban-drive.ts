...
export function classifyProject(idea: string): ProjectType {
  ...
  if (/\b(maintenance|vendor|owner|tenant|lease|leasing|inspection|property|dispatch)\b/.test(i))
    return "pm-workflow";
...
const UNSAFE =
  /\b(deploy|production|billing|charge|credential|secret|delete|drop|destroy|send|email|sms|outreach|customer)\b/i;
...
export function planFloors(idea: string, graphFiles: string[] = []): DrivePlan {
  ...
  const tools = [
    "Graphify",
    "Skills Library",
    projectType === "pm-workflow" ? "Comms (dry-run safe)" : "Editor",
  ];
  ...
  if (UNSAFE.test(idea))
    approvalGates.push(
      "Safety gate — touches a protected action (deploy/billing/creds/external/delete)",
    );
  ...
  const floors: FloorOutput[] = [
    {
      floor: 1,
      name: "Intake",
      output: `Clarified idea · type=${projectType} · area=${productArea}`,
    },
    {
      floor: 2,
      name: "Architecture",
      output: `Graphify located ${files.length} files; dependencies mapped`,
    },
    {
      floor: 3,
      name: "Execution Plan",
      output: `${steps.length} steps · agents: ${agents.join(", ")}`,
    },
    {
      floor: 4,
      name: "Cost / Risk / Strategy",
      output: `~${costFor(projectType)} tokens · gates: ${approvalGates.length}`,
    },
  ];
  const payloadSpec: PayloadSpec = {
    ...
    agents,
    tools,
    costEstimateTokens: costFor(projectType),
    approvalGates,
    graphFiles: graphFiles.slice(0, 6),
  };
  return {
    idea,
    floors,
    payloadSpec,
    summary: `${projectType} in ${productArea}: ${steps.length} steps, ${agents.length} agents, ${approvalGates.length} approval gate(s).`,
  };
}

function costFor(t: ProjectType): number {
  return t === "pm-workflow" || t === "dashboard"
    ? 8000
    : t === "blog" || t === "report"
      ? 5000
      : 3000;
...
export function selfCheck(spec: PayloadSpec, artifact: string): SelfCheckResult {
  ...
  const checks = [
    ...
    {
      name: "covers planned steps",
      ok: spec.steps.some((s) => a.includes(s.toLowerCase().split(" ")[0])) || a.length > 80,
    },
    {
      name: "references a target file",
      ok: spec.files.some((f) => a.includes(f.split("/").pop()!.toLowerCase())) || a.length > 120,
    },
  ];
  ...
  return {
    ...
    checks,
    logs: fails.length
      ? `FAIL: ${fails.map((f) => f.name).join("; ")}`
      : "PASS: artifact satisfies payload spec",
  };
}
...
export function nextStage(
  stage: Stage,
  opts: { approved?: boolean; checkPass?: boolean; attempts?: number } = {},
): Stage {
...
export function driveReplayEvents(plan: DrivePlan, now = 0): ReplayEvent[] {
  const ev: ReplayEvent[] = [
    { ts: now, kind: "trigger", label: `/drive: ${plan.idea}`.slice(0, 80) },
  ];
  if (plan.payloadSpec.graphFiles.length)
    ev.push({
      ts: now,
      kind: "tool_call",
      agent: "PI Agent",
      label: "Graphify query (graph-first)",
      detail: `${plan.payloadSpec.graphFiles.length} files`,
    });
  for (const f of plan.floors)
    ev.push({
      ...
      kind: "agent_start",
      agent: `Floor ${f.floor}`,
      label: `${f.name}: ${f.output}`.slice(0, 90),
    });
  ev.push({
    ts: now,
    kind: "approval",
    label: "Awaiting human approval",
    detail: plan.payloadSpec.approvalGates.join("; "),
  });
  return ev;
}

/** Obsidian memory entry (markdown) for a shipped card. */
export function obsidianMarkdown(card: {
  projectName: string;
  idea: string;
  plan: DrivePlan;
  ...
  return [
    ...
    ``,
    `## 5-Floor Plan`,
    ...card.plan.floors.map((f) => `- **Floor ${f.floor} ${f.name}** — ${f.output}`),
    ``,
    `## Payload spec`,
    ...
    `## Proof / Replay`,
    `- Proof: ${card.proofId ?? "—"}`,
    `- Replay: ${card.replayId ?? "—"}`,
    ``,
    `## Artifact`,
    "```",
    card.artifact.slice(0, 4000),
    "```",
    ``,
  ].join("\n");
}
