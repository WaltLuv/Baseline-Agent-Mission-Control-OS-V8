...
export const Route = createFileRoute("/memory")({
  head: () => ({
    meta: [
      { title: "Memory — Baseline Automations" },
      {
        name: "description",
...
// All four memory layers are ALWAYS visible in the graph filter pills.
// Pinecone is part of the stack even when no index is configured — clicking
// the pill opens a setup CTA so the user knows what to do.
const BASE_SOURCES = ["obsidian", "claude", "notion", "pinecone"] as const;
const PINECONE_SOURCES = BASE_SOURCES;
type SourceId = "obsidian" | "claude" | "notion" | "pinecone";

function MemoryPage() {
  const [selected, setSelected] = useState<MemNode | null>(null);
  const liveData = useLiveData();
  const ld = liveData as any;
  // hasPinecone is now only used to decide whether the pill renders the
  // configured count or the "configure" CTA — Pinecone is ALWAYS in the
  // source list (see BASE_SOURCES above).
  const hasPinecone = (ld?.memory?.stats?.pineconeIndexes ?? 0) > 0 || ld?.detection?.memoryStores?.pinecone?.hasKey === true;
  const ALL_SOURCES = PINECONE_SOURCES;

  // Notion memory layer — pulled live from /__notion_search.
  const [notionNodes, setNotionNodes] = useState<{ id: string; label: string; source: "notion"; kind: "file"; url?: string }[]>([]);
  const [pineconeNodes, setPineconeNodes] = useState<{ id: string; label: string; source: "pinecone"; kind: "file"; url?: string }[]>([]);
  const [notionLoaded, setNotionLoaded] = useState(false);
  useEffect(() => {
    let cancel = false;
    fetch("/__notion_search", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({}) })
      .then((r) => r.json())
      .then((j: { results?: { id: string; object: string; url?: string; properties?: Record<string, { type?: string; title?: { plain_text?: string }[] }>; title?: { plain_text?: string }[] }[] }) => {
        if (cancel) return;
        const nodes = (j.results ?? []).map((item) => {
          let label = "(untitled)";
          if (item.object === "page" && item.properties) {
            for (const v of Object.values(item.properties)) {
              if (v?.type === "title" && v.title) { label = v.title.map((t) => t.plain_text ?? "").join("") || label; break; }
            }
          } else if (item.object === "database" && item.title) {
            label = item.title.map((t) => t.plain_text ?? "").join("") || label;
          }
          return { id: `notion-${item.id}`, label, source: "notion" as const, kind: "file" as const, url: item.url };
        });
        setNotionNodes(nodes);
        setNotionLoaded(true);
      })
      .catch(() => { if (!cancel) setNotionLoaded(true); });

    // Pinecone — represent each populated namespace as a node, since pulling
    // every individual vector would be heavy. Stats endpoint returns counts.
    fetch("/__pinecone_status")
      .then((r) => r.json())
      .then((j: { ok?: boolean; indexName?: string; totalVectors?: number; namespaces?: Record<string, { vectorCount: number }> }) => {
        if (cancel || !j.ok) return;
        const nodes: typeof pineconeNodes = [];
        if (j.namespaces && Object.keys(j.namespaces).length > 0) {
          for (const [ns, info] of Object.entries(j.namespaces)) {
            nodes.push({
              id: `pinecone-ns-${ns || "default"}`,
              label: `${ns || "default"} · ${info.vectorCount} vectors`,
              source: "pinecone" as const,
              kind: "file" as const,
              url: undefined,
            });
          }
        } else if (j.totalVectors && j.totalVectors > 0) {
          nodes.push({
            id: "pinecone-default",
            label: `${j.indexName ?? "pinecone"} · ${j.totalVectors} vectors`,
            source: "pinecone" as const,
            kind: "file" as const,
          });
        }
        setPineconeNodes(nodes);
      })
      .catch(() => { /* skip — Pinecone not configured is a normal state */ });
  ...
  const matchesActive = (sourceTag: string | undefined, kind?: string) => {
    ...
    if (sourceTag === "notion" && activeSet.has("notion")) return true;
    return false;
  };
    ...
    <div className="max-w-[1400px]">
      <header className="flex items-end justify-between border-b border-border pb-9 mb-7">
        <div>
          ...
          <div className="mt-3 max-w-2xl"><GraphifyAwareness context="knowledge memory brain" /></div>
        </div>
      </header>
      ...
      <section className="rounded-xl border border-border bg-card overflow-hidden mb-10 relative">
        ...
        >
          <Suspense fallback={<MemoryGraphLoader height={640} />}>
            <MemoryGraph3D onSelect={setSelected} sourceFilter={graphFilter} extraNodes={[...notionNodes, ...pineconeNodes]} />
          </Suspense>
        </div>
      </section>
...
function SourceFilter({
  ...
  const pills: Pill[] = [
    ...
    },
    {
      ...
      label: "Notion",
      color: "#f0f0f0",
      tooltip: "Notion pages shared with the Slim Charles Memory Layer integration",
    },
    // Pinecone is ALWAYS visible — even before the user creates an index.
    // When unconfigured the sub-label flips to "configure →" so the empty
    // state is the call-to-action, not a missing pill.
    {
      id: "pinecone" as const,
      label: "Pinecone",
      sub: pineconeCount
        ? `${pineconeCount} indexes`
        : (liveData?.detection?.memoryStores?.pinecone?.hasKey ? undefined : "configure →"),
      logo: pineconeIconSvg,
      mask: true,
      color: "#22D3EE",
      tooltip: pineconeCount
        ? "Vector indexes — every Pinecone collection feeds this memory source"
        : "Pinecone not configured yet — click to set PINECONE_API_KEY + PINECONE_INDEX_HOST in .env.local",
    },
  ];

  // When Pinecone is unconfigured, the click navigates to /pinecone (the
  // setup page) instead of toggling the filter — there's nothing to filter yet.
  const pineconeUnconfigured = !pineconeCount && !liveData?.detection?.memoryStores?.pinecone?.hasKey;

  return (
    <div className="mb-5 flex flex-wrap gap-2">
      {pills.map((p) => {
        const isActive = p.id === "all" ? allOn : activeSet.has(p.id);
        const goToSetup = p.id === "pinecone" && pineconeUnconfigured;
        return (
          <button
            key={p.id}
            onClick={() => {
              if (goToSetup) {
                window.location.href = "/pinecone";
                return;
              }
              onToggle(p.id);
            }}
