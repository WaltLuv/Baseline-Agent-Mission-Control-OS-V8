...
function inferCategory(name: string): (typeof skillCategories)[number] {
  ...
  if (n.includes("video") || n.includes("script") || n.includes("intro") || n.includes("title"))
    return "Video";
  if (n.includes("page") || n.includes("design") || n.includes("3d") || n.includes("render")) return "Design";
  if (n.includes("organize") || n.includes("indexer") || n.includes("automation") || n.includes("pipeline"))
    return "Automation";
  if (n.includes("report") || n.includes("cost") || n.includes("snapshot")) return "Reporting";
  return "Coding";
}

// Map the raw `cat` value from SKILL_INDEX.json onto the 8 UI categories.
function mapLibraryCategory(cat: string | undefined, name: string): (typeof skillCategories)[number] {
  const c = (cat ?? "").toLowerCase();
  if (!c) return inferCategory(name);
  if (c === "creative" || c === "design") return "Design";
  if (c === "media" || c === "video") return "Video";
  if (c === "research" || c === "knowledge") return "Research";
  if (c === "sales" || c === "marketing" || c === "reporting") return "Reporting";
  if (c === "communication") return "Memory";
  if (c === "review" || c === "audit" || c === "qa") return "Review";
  if (c === "memory" || c === "rag") return "Memory";
  if (c === "propcontrol" || c === "orchestration" || c === "productivity" || c === "automation") return "Automation";
  if (c === "devops" || c === "mlops" || c === "data-science" || c === "coding") return "Coding";
  // anything else falls back to name inference
...
function librarySkillsToRows(items: LibrarySkill[], usedNames: Set<string>): SkillRow[] {
  return items
    // Skip skills that match what the aggregator already produced from JSONL,
    // so /dream / /personas / etc. aren't duplicated when the library also ships them.
    .filter((s) => !usedNames.has(s.name) && !usedNames.has(`/${s.name}`))
    .map((s): SkillRow => ({
      name: s.name,
      category: mapLibraryCategory(s.cat, s.name),
      scope: "global",
      workspace: null,
      lastUsed: "library",
      status: "unused",
      inputs: s.tags?.slice(0, 2) ?? [],
      outputs: [],
      uses: 0,
      // Heuristic score: well-documented or master-doctrine skills surface higher.
      score: /master|locked|doctrine|24-pipeline/i.test(s.name) ? 75 : 60,
    }));
}
...
export const Route = createFileRoute("/skills")({
  head: () => ({
    meta: [
      { title: "Skills — Baseline Automations" },
      {
        name: "description",
...
function SkillsPage() {
  ...
  const liveSkills = buildSkillsFromLive();
  const [libSkills, setLibSkills] = useState<SkillRow[]>([]);
  useEffect(() => {
    fetch("/__skills_shared")
      .then((r) => r.json())
      .then((j: { skills?: LibrarySkill[] }) => {
        const usedNames = new Set(liveSkills.map((s) => s.name));
        setLibSkills(librarySkillsToRows(j.skills ?? [], usedNames));
      })
      .catch(() => setLibSkills([]));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Merge: live-used skills first (so /dream, /personas, /yuanbao stay at the
  // top), then the 230-item library underneath.
  skills = [...liveSkills, ...libSkills];
  ...
  const byCategory = useMemo(
    ...
    // libSkills + liveSkills drive the contents of `skills`. Recompute when either changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [libSkills.length, liveSkills.length],
  );
  ...
  const topSavers = useMemo(() => {
    return skills
      ...
      .slice(0, 6);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [minutesFor, rate, period, libSkills.length, liveSkills.length]);
...
// ─── DreamRunCta — prominent "Run /dream now" hero card on Skills page ──────
// Surfaces the daily Dream review as the headline command, with a Run-Now
// button that POSTs to /__dream_run, streams the output, and reloads on
// completion so the dashboard picks up the fresh prescription file.
function DreamRunCta() {
  ...
  return (
    <section
      className="relative rounded-xl overflow-hidden border mb-10 p-6 md:p-7"
      style={{
        background:
          "linear-gradient(120deg, rgba(167,139,250,0.16) 0%, rgba(99,102,241,0.10) 45%, rgba(7,29,28,0.6) 100%)",
        borderColor: "rgba(167,139,250,0.35)",
        boxShadow: "0 0 28px -8px rgba(167,139,250,0.35)",
      }}
    >
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-[10.5px] uppercase tracking-[0.28em] text-violet-200/85 mb-1.5">
            <Sparkles className="h-3.5 w-3.5" /> Daily Dream Review
          </div>
          <h2 className="text-xl md:text-2xl font-semibold tracking-tight text-violet-50">
            /dream — the headline command
          </h2>
          <p className="text-[12.5px] text-violet-100/75 mt-1.5 max-w-2xl leading-relaxed">
            Audits the last 24h of your AI stack across 8 signal buckets — conversation, cost,
            skills, memory, sessions, workflows, business, and external — and writes the four
            highest-impact prescriptions to{" "}
            <code className="text-violet-200">~/.claude-os/dreams/dream-&#123;date&#125;.json</code>.
            Runs nightly at 7am; run it on-demand here.
          </p>
        </div>
        <div className="shrink-0 flex flex-col items-stretch gap-2">
          <button
            onClick={run}
            disabled={running}
            className="inline-flex items-center justify-center gap-2 rounded-lg px-5 py-3 text-[13px] font-semibold tracking-tight border transition disabled:opacity-60 disabled:cursor-not-allowed"
            style={{
              background: running ? "rgba(167,139,250,0.18)" : "rgba(167,139,250,0.28)",
          ...
          >
            {running ? (
              <><span className="inline-block h-2 w-2 rounded-full bg-violet-200 animate-pulse" /> running /dream…</>
            ) : (
              <><Play className="h-4 w-4" /> Run /dream now</>
            )}
          </button>
          <span className="text-[10px] uppercase tracking-[0.22em] text-violet-200/55 text-center">
            ~60–120s · writes to disk · reloads dashboard
          </span>
        </div>
      </div>
      {log && (
        <pre className="mt-4 max-h-[220px] overflow-y-auto rounded-lg border border-violet-300/20 bg-black/45 p-3 text-[10.5px] leading-relaxed text-violet-100/90 whitespace-pre-wrap font-mono">
          {log.slice(-3500)}
        </pre>
      )}
    </section>
