...
type SubTool = "create" | "avatar" | "workspace";
...
export function VideoStudio() {
  const [tool, setTool] = useState<SubTool>("create");
  return (
      ...
      <div className="relative overflow-hidden rounded-xl border p-4"
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            ...
            <div className="text-[15px] font-medium text-[var(--cream)]">HyperFrames + AI Avatar</div>
          </div>
          <div className="ml-auto text-[10.5px] uppercase tracking-widest text-[var(--cream-mute)]">
            local · 0 cents per render
          </div>
        </div>
        <p className="text-[12px] text-[var(--cream-dim)] max-w-[640px]">
          Generate videos with the HyperFrames CLI (HTML → MP4) or AI avatars (text → talking head).
          Everything saves locally and shows up in Workspace.
        </p>
      </div>
...
function CreateTab() {
  ...
  async function loadProjects() {
    try {
      const r = await fetch("/__hyperframes_projects", { cache: "no-store" });
  ...
  async function loadJobs() {
    try {
      const r = await fetch("/__hyperframes_render_status", { cache: "no-store" });
      const j = await r.json();
      setJobs(j.jobs ?? []);
    } catch {}
  }
  async function refreshOpenJob() {
    if (!openJob) return;
    try {
      const r = await fetch(`/__hyperframes_render_status?id=${encodeURIComponent(openJob.job.id)}`, { cache: "no-store" });
      const j = await r.json();
  ...
  async function createAndRender() {
    ...
    try {
      const init = await fetch("/__hyperframes_init", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt }),
      }).then((r) => r.json());
      if (!init.ok) { setError(init.error || "Init failed"); setBusy(false); return; }
      const r = await fetch("/__hyperframes_render", {
        method: "POST", headers: { "Content-Type": "application/json" },
...
function AvatarTab() {
  ...
  const [avatarQuery, setAvatarQuery] = useState("");  // default — search your own clones
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      try {
        const r = await fetch(`/__heygen_avatars?q=${encodeURIComponent(avatarQuery)}&limit=40`);
        const j = await r.json();
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      try {
        const r = await fetch(`/__heygen_voices?q=${encodeURIComponent(voiceQuery)}&limit=40`);
        const j = await r.json();
        setVoices(j.voices ?? []);
      } catch {}
    }, 300);
  ...
  async function pollJob(videoId: string) {
    try {
      const r = await fetch(`/__heygen_status?id=${encodeURIComponent(videoId)}`);
      const j = await r.json();
      if (j.ok) {
  ...
  async function generate() {
    ...
    setBusy(true); setError(null);
    try {
      const r = await fetch("/__heygen_generate", {
        method: "POST",
      ...
      <PreviewCard title={job ? `Avatar video · ${job.status}` : "Preview"} accent={ACCENT}>
        {job ? (
          ...
          <EmptyState icon={<User size={28} style={{ color: ACCENT }} />} title="Pick an avatar + voice → script → Generate"
            hint="AI avatar renders typically take 30–120 seconds. Search to filter your own avatar + voice clones. Status polls every 5s." />
        )}
      </PreviewCard>
...
function WorkspaceTab() {
  ...
  async function loadBuckets() {
    try {
      const r = await fetch("/__video_workspace", { cache: "no-store" });
      const j = await r.json();
  ...
  async function loadFiles(id: string) {
    try {
      const r = await fetch(`/__video_workspace?bucket=${encodeURIComponent(id)}`, { cache: "no-store" });
      const j = await r.json();
      setFiles(j.files ?? []);
    } catch { setFiles([]); }
  }
  ...
  return (
    <div className={`grid grid-cols-1 ${open ? "lg:grid-cols-[260px_280px_1fr]" : "lg:grid-cols-[320px_1fr]"} gap-0 h-full min-h-[600px] border rounded-md overflow-hidden`}
      ...
      {selected && open && (() => {
        ...
        <section className="flex flex-col min-h-0 overflow-hidden border-l" style={{ borderColor: "var(--line-soft)" }}>
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
            {k === "video" && (
              <div className="flex-1 grid place-items-center bg-black/40 p-2">
                <video src={open.url} controls autoPlay className="max-w-full max-h-full" />
              </div>
            )}
            {k === "audio" && (
