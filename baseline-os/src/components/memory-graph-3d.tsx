...
export function MemoryGraph3D({
  ...
  embedded?: boolean;
  sourceFilter?: string;
  /** Extra nodes from runtime sources (e.g. Notion) — wired in alongside the base graph. */
  extraNodes?: { id: string; label: string; source: string; kind: "file"; url?: string }[];
}) {
  ...
  // Merge runtime nodes from additional sources (Notion, Pinecone, …).
  // Each *source* gets its own hub node + its own cluster of file/vector nodes.
  const SOURCE_PALETTE: Record<string, { fill: string; ring: string; label: string }> = {
    notion:   { fill: "#f0f0f0", ring: "#bdb5ad", label: "Notion" },
    pinecone: { fill: "#22D3EE", ring: "#0891b2", label: "Pinecone" },
  };
  const mergedFullData = useMemo(() => {
    const nodes = [...fullData.nodes];
    const links = [...fullData.links];
    if (extraNodes && extraNodes.length > 0) {
      // Group by source so each source gets one cluster + one hub.
      const grouped = new Map<string, typeof extraNodes>();
      for (const n of extraNodes) {
        const arr = grouped.get(n.source) ?? [];
        arr.push(n);
        grouped.set(n.source, arr);
      }
      for (const [src, items] of grouped) {
        const palette = SOURCE_PALETTE[src] ?? { fill: "#9aa3b0", ring: "#6b7280", label: src };
        const hubId = `${src}-hub`;
        nodes.push({
          id: hubId,
          name: palette.label,
          ...
          val: 5.5,
          color: palette.ring,
          meta: `${items.length} ${src === "pinecone" ? "namespace(s)" : "items"}`,
        });
        links.push({ source: "hub", target: hubId, kind: "core" });
        for (const n of items) {
          nodes.push({
            ...
            kind: "file",
            source: src,
            val: src === "pinecone" ? 2.2 : 1.6, // pinecone "namespace" nodes are slightly bigger
            color: palette.fill,
            meta: n.url ?? "",
            preview: n.url,
          });
          links.push({ source: hubId, target: n.id, kind: "file" });
        }
      }
    }
    return { nodes, links };
  ...
  const data = useMemo(() => {
    let nodes = mergedFullData.nodes;
    let links = mergedFullData.links;
    if (sourceFilter && sourceFilter !== "all") {
      ...
      const matches = (n: MemNode): boolean => {
        if (allowedCats.has("pinecone") && (n.kind === "vector_store" || n.source === "pinecone" || n.id === "pinecone-hub"))
          return true;
        if (allowedCats.has("obsidian") && n.source === "obsidian") return true;
        if (allowedCats.has("claude") && n.source === "claude") return true;
        if (allowedCats.has("notion") && (n.source === "notion" || n.id === "notion-hub")) return true;
        return false;
      };
  ...
  }, [mergedFullData, density, sourceFilter]);
