...
```
Install Baseline OS for me. Ask once for permission, then run all four
commands in order without further prompts:

1. git clone https://github.com/WaltLuv/baseline-agent-os.git ~/code/claude-os
2. cd ~/code/claude-os && bun install
3. cd ~/code/claude-os && bun run setup
4. cd ~/code/claude-os && bun run dev

Step 3 (`bun run setup`) does the heavy lifting on my behalf:
...
Step 4 (`bun run dev`) stays running and pops the 7-step wizard in
my browser.

Once the wizard is open, send this line:

  ✨ Baseline OS is ready. The wizard is open in your browser — walk
...
A) AUTO-DETECTED — Baseline OS already saw it (Obsidian vaults under
...
I'll show you what Baseline OS already sees, what needs a quick
...
- **Dream panel** — four prescription cards (memory / cost / skills / workflow) with a `[Run this fix →]` button that sends the prescription ID to the local sidecar, which resolves it from the latest Dream JSON and runs the stored `claude -p` command. If the sidecar is offline, the UI falls back to copying the command. Source: `~/.claude-os/dreams/dream-{date}.json`.
...
| Feature                | Status                                             |
| ---------------------- | -------------------------------------------------- |
| `bun create claude-os` | 🔴 Package not yet on npm — clone the repo for now |
...
| 2   | Detect tools  | Probes for Claude Code / Desktop / VS Code / AntiGravity / Codex / Gemini / Anthropic API / OpenAI API / OpenRouter / OpenClaw. On a fresh clone, auto-runs the aggregator via the sidecar (`POST /api/aggregate`) the first time you land on this step, then reloads with real detection results. |
...
`security dump-keychain` (used to estimate Claude plan tier) will additionally prompt for your **login password** the first time it runs. That's a macOS prompt — Baseline OS never sees the password.
...
- **Source data stays read-only.** Baseline OS never writes to source tool data directories like `~/.claude/`, `~/.codex/`, `~/.openclaw/`, or your Obsidian vaults. The sidecar writes Baseline OS config to `~/.claude-os/config.json`, writes repo-local `.env.local`, and the aggregator writes `src/data/live-data.json`.
- **Local processing.** No telemetry. No phone-home. No analytics. The only outbound calls are the APIs whose keys you explicitly provide (OpenRouter, Pinecone) and Dream runs you invoke via `claude -p`, which make Anthropic API calls through Claude Code.
...
Dream is what makes Baseline Automations more than a metrics dashboard.

A daily 7am cron runs `claude -p "/dream"`, which audits your last 24 hours of activity across **eight orthogonal signal buckets**:
...
The cron installer writes `~/Library/LaunchAgents/com.claude-os.dream.plist`, loads it via `launchctl`, and pipes output to `~/.claude-os/dream-cron.log`. The skill itself lives at `skills/dream/SKILL.md` in this repo and is what `claude -p "/dream"` runs. On Linux/Windows the installer prints a `crontab` snippet you can install by hand.
...
Personal & Commercial Use License with Attribution. See [`LICENSE`](LICENSE). You may use and modify Baseline Automations for personal use AND commercial work (client projects, freelance, day job, internal tooling). You must credit "Baseline Automations by Walter Thornton" on any public deliverable it materially contributed to. You may not redistribute, repackage, sell, or re-upload it.
