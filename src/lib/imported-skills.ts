...
export const IMPORTED_SKILLS: ImportedSkill[] = [
  ...
  },
  {
    ...
    summary: 'Programmatic NotebookLM: create notebooks, add sources (URL/YouTube/PDF/audio), generate artifacts, download formats.',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'review',
  ...
  },
  {
    ...
    category: 'Creative',
    source: 'SKILL.md',
    summary: 'Convert NotebookLM slide decks (PDF) into editable PPTX / PNG / HTML via the NotebookLM MCP.',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'auto',
  ...
  },
  {
    ...
    summary: 'Generate self-contained HTML slide decks (style/density/images configurable) — one .html file, no dependencies.',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'auto',
    requiredCredentials: [],
  ...
  },
  {
    ...
    category: 'Content',
    source: 'YouTube.md',
    summary: 'Scrape a channel’s transcripts via the YouTube Data API + yt-dlp (.vtt), then feed content workflows.',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'review',
    requiredCredentials: ['youtube_api', 'yt_dlp'],
    proofExpectations: 'Per-video .vtt transcripts + a structured content index.',
    wiresInto: ['Claude Code Studio', 'Video Editing Team', 'Creative Provider Matrix', 'Publishing checklist'],
  },
  {
    ...
    summary: 'End-to-end: push a web project to GitHub and deploy it live on Vercel (handles the known gotchas).',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'walt-only',
    requiredCredentials: ['github', 'vercel'],
    proofExpectations: 'New/updated GitHub repo + live Vercel URL (approval required before deploy).',
  ...
  },
  {
    ...
    summary: 'Personalized animated HTML morning-brief dashboard (priority emails, calendar, news, action items, stats).',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'auto',
  ...
  },
  {
    ...
    summary: 'Recall permanent context from the vector memory before answering (semantic search over stored memories).',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'auto',
  ...
  },
  {
    ...
    summary: 'At session end, summarize decisions/progress and store them permanently (chunked, deduped, no secrets indexed).',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'auto',
    requiredCredentials: ['pinecone'],
  ...
  },
  {
    ...
    summary: 'Maintain a living strategy document in long-term memory, recalled and updated across sessions.',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'review',
    requiredCredentials: ['pinecone'],
  ...
  },
  {
    ...
    summary: 'Permanent vector-memory architecture: chunk → embed → dedupe → upsert → query, with proof and no secrets indexed.',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'auto',
    requiredCredentials: ['pinecone'],
  ...
  },
  {
    ...
    summary: 'Drive NotebookLM research from the Antigravity multi-agent runtime (notebook chat → synthesized output).',
    pricing: 'free',
    priceUsd: 0,
    approvalTier: 'review',
    requiredCredentials: ['notebooklm', 'antigravity'],
    proofExpectations: 'Antigravity-driven research run + NotebookLM-sourced synthesis with citations.',
    wiresInto: ['NotebookLM Brain Layer 4', 'Antigravity'],
  },
]

const CATEGORIES: ImportedCategory[] = ['Knowledge', 'Creative', 'Content', 'Memory', 'DevOps', 'Ops', 'AI Agents', 'Data']

export function importedSkillsByCategory(): Record<ImportedCategory, ImportedSkill[]> {
  const out = Object.fromEntries(CATEGORIES.map((c) => [c, [] as ImportedSkill[]])) as Record<ImportedCategory, ImportedSkill[]>
