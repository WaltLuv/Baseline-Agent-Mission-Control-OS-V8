...
export function buildGraphContext(graph: KnowledgeGraph | null, task: string, now = 0): GraphContext {
  if (!graph || !task.trim()) {
    return { query: task, nodes: [], files: [], dependencies: [], confidence: 0, promptBlock: '', replayEvents: [] }
  }
  ...
  const confidence = nodes.length === 0 ? 0 : Math.min(1, Math.round((nodes.length / 8) * 100) / 100)
  ...
  const replayEvents: ReplayEvent[] = nodes.length
    ? [
        { ts: now, kind: 'tool_call', agent: 'PI Agent', label: `Graphify query: ${task}`.slice(0, 80), detail: `${nodes.length} nodes` },
        { ts: now, kind: 'file_touched', agent: 'PI Agent', label: `located ${files.length} files`, detail: files.slice(0, 6).join(', ') },
      ]
    : []
...
export async function fetchGraphContext(task: string, endpoint = '/api/graphify'): Promise<GraphContext | null> {
  try {
